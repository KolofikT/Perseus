...
void setup() {
    ...
    cfg.prevod_motoru = 901.0f; // Korekce: 1012 × (1000/1123) — sníženo, robot přejížděl
        ...
        cfg.konstanta_radius_vnejsi_kolo = 0.96f; // Korekce pro vnější kolo při zatáčení
        cfg.konstanta_radius_vnitrni_kolo = 0.96f; // Korekce pro vnitřní kolo při zatáčení
        cfg.korekce_nedotacivosti_left = 0.97f; // Korekce při otáčení na místě doleva
        cfg.korekce_nedotacivosti_right = 0.97f; // Korekce při otáčení na místě doprava

        // - Motory
        cfg.motor_id_left = 4; // ID levého motoru na desce
        cfg.motor_id_right = 1; // ID pravého motoru na desce
        cfg.motor_max_power_pct = 100; // Limit výkonu motorů (0-100%)
        cfg.motor_polarity_switch_left = true; // Prohození polarity levého motoru
        cfg.motor_polarity_switch_right = false; // Prohození polarity pravého motoru
        cfg.motor_enable_failsafe = false; // Zastaví motory po 500ms, pokud nepřijde příkaz
        cfg.motor_wheel_diameter = 68.0f; // Průměr kol v mm (pro hrubý výpočet)
        cfg.motor_max_ticks_per_second = 5200; // Maximální rychlost (nutno změřit pomocí max_rychlost())
        cfg.motor_max_acceleration = 10000; // Sníženo z 50000 pro plynulejší rozjezd (koprocesor nebude skákat na cílovou rychlost)

        // - Serva 
        cfg.stupid_servo_min = -1.65f; // Spodní hranice signálu hloupých serv
        cfg.stupid_servo_max = 1.65f; // Horní hranice signálu hloupých serv
        cfg.pocet_chytrych_serv = 7;
        delay(500);

        // - Tlačítka (GPIO piny) a WiFi
        cfg.Button1 = 0; // Pin zadního tlačítka 1 (0 = vypnuto)
        cfg.Button2 = 0; // Pin zadního tlačítka 2 (0 = vypnuto)
        cfg.enable_wifi_log = false; // Povolí výpisy na web (printf_wifi)
        cfg.enable_wifi_control_wasd = false; // Povolí dálkové ovládání přes web
        cfg.enable_wifi_terminal = false; // Povolí ovládání příkazy z webu
        cfg.wifi_ssid = "robot1234"; // Jméno vysílané/připojované WiFi
        cfg.wifi_password = "1234robot"; // Heslo WiFi
...
// ======= TESTOVACÍ ČÁSTI ==========
    ...
    //     int distRightRear = rkUltraMeasure(3); // Měření vzdálenosti z pravého zadního ultrazvuku (U3)
    //     int distLeftRear  = rkUltraMeasure(4); // Měření vzdálenosti z levého zadního ultrazvuku (U4)

    //     if (color_ok) {
    //         printf("[Senzory] Laser: %4d mm | PravyZadni(U3): %4d mm | LevyZadni(U4): %4d mm | Barva RGB: (%3.0f, %3.0f, %3.0f)\n", dist, distRightRear, distLeftRear, r, g, b);
    //     } else {
    //         printf("[Senzory] Laser: %4d mm | PravyZadni(U3): %4d mm | LevyZadni(U4): %4d mm | Barva RGB: Chyba cteni\n", dist, distRightRear, distLeftRear);
    //     }
    // }


    // printf("Napětí: %u mV\n", man.battery().voltageMv());
